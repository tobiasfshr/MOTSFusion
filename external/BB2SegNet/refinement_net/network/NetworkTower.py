...
from external.BB2SegNet.refinement_net.core.Util import import_submodules
...
def get_layer_class(layer_class):
  if not hasattr(get_layer_class, "_imported"):
    ...
    import_submodules("external.BB2SegNet.refinement_net.network")
...
class NetworkTower:
  def __init__(self, config, tower_setup, input_tensors_dict, dataset):
    ...
    for layer in self.layers.values():
      ...
      # each measure can only be used once in the network for now
      for k in layer.measures.keys():
        assert k not in self.measures
      self.measures.update(layer.measures)
      # each extraction can only be used once in the network for now
