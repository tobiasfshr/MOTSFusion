...
from external.BB2SegNet.refinement_net.network.ConvolutionalLayers import Conv
from external.BB2SegNet.refinement_net.network.Util import max_pool, prepare_input
...
class ResNet50(Layer):
  def __init__(self, name, inputs, tower_setup):
    ...
    with tf.variable_scope(name):
      self.outputs = pretrained_resnet50_conv4(inp, tower_setup)
      self.outputs.append(add_resnet_conv5(self.outputs[-1], tower_setup)[-1])
