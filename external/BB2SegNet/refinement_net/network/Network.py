...
from external.BB2SegNet.refinement_net.network.NetworkTower import NetworkTower, TowerSetup
  ...
  def _build_towers(self, config, dataset, is_trainnet, freeze_batchnorm, reuse_variables):
    ...
    with tf.name_scope(self.name):
      for gpu_idx in range(n_gpus):
        ...
        # print("yoyoyoyoyooyoyooyo", reuse_variables,is_trainnet,gpu_idx,not is_trainnet or gpu_idx > 0)
        if gpu_idx > 0:
          reuse_variables = True
        elif reuse_variables is None:
          reuse_variables = not is_trainnet or gpu_idx > 0
        tower_setup = TowerSetup(gpu_idx=gpu_idx, reuse_variables=reuse_variables, dataset=dataset,
