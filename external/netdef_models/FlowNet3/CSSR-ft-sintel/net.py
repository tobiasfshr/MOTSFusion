...
from netdef_slim.architectures import Architecture_C, Architecture_S, Architecture_R
    ...
    def make_graph(self, data, include_losses=True):
        ...
        with nd.Scope('net2', learn=False, **self.scope_args()):
            ...
            arch2 = Architecture_S(
                num_outputs=pred_config.total_channels(),
                disassembling_function=pred_config.disassemble,
                loss_function= None, #(lambda pred, level, weight: self.make_total_loss(pred, data, level, weight)) if include_losses else None,
                conv_upsample=self._conv_upsample
            )
        ...
        #### Refine Net ####
        flow_fwd = out3.final.flow[0].fwd
        occ_fwd = out3.final.occ[0].fwd
        mb_fwd = out3.final.mb[0].fwd
        warped = nd.ops.warp(data.img[1], flow_fwd)

        error = nd.ops.spatial_epe(data.img[0], warped)
        mag = nd.ops.channel_norm(flow_fwd)

        input_r = nd.ops.concat([
            data.img[0],
            error,
            mag,
            out3.final.flow[0].fwd,
            out3.final.occ[0].fwd,
            out3.final.mb[0].fwd
        ])
        ...
        pred_config[0].mod_func = lambda x: nd.ops.add(x, nd.ops.resample(flow_fwd, reference=x, type='LINEAR', antialias=False))
        pred_config[1].mod_func = lambda x: nd.ops.add(x, nd.ops.resample(occ_fwd, reference=x, type='LINEAR', antialias=False))
        pred_config[2].mod_func = lambda x: nd.ops.add(x, nd.ops.resample(mb_fwd, reference=x, type='LINEAR', antialias=False))

        with nd.Scope('net_r', learn=True, **self.scope_args()):
            arch_r = Architecture_R(
                num_outputs=pred_config.total_channels(),
                disassembling_function=pred_config.disassemble,
                loss_function=None,
                conv_upsample=self._conv_upsample,
            ...
            out_r = arch_r.make_graph(input_r)
        return out_r
