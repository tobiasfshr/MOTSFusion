...
import eval.mot_eval.mailpy as mailpy
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, list_seq, gt_path="./eval/mot_eval/data/tracking", min_overlap=0.5, max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None, cls="car"):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        filename_test_mapping = os.path.join(gt_path, "evaluate_tracking.seqmap.training")
        self.sequence_name = []
        self.n_frames = []
        ...
        for i in reversed(range(len(self.sequence_name))):
            if self.sequence_name[i] not in list_seq:
                self.sequence_name.pop(i)
                self.n_frames.pop(i)

        self.n_sequences = len(self.sequence_name)+1
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            if loading_groundtruth:
                filename = os.path.join(os.path.join(root_dir, s_name), s_name + '.txt')
            else:
                filename = os.path.join(os.path.join(root_dir, s_name), "mot_result")
            f              = open(filename, "r")
    ...
    def compute3rdPartyMetrics(self, eval_modified):
        """
        ...
        for seq_idx, (seq_trajectories,seq_ignored) in enumerate(zip(self.gt_trajectories, self.ign_trajectories)):
            ...
            n_ignored_tr = 0
            for g, ign_g in zip(list(seq_trajectories.values()), list(seq_ignored.values())):
    ...
    def saveToStats(self, mail):
        ...
        dump = open(filename, "w+")
        print("MOTA", "MOTP", "MOTAL", "MODA", "MODP", "recall", "precision", "F1", "FAR", end=' ', file=dump)
        print("MT", "PT", "ML", "tp", "fp", "fn", "id_switches", "fragments", end=' ', file=dump)
        print("n_gt", "n_gt_trajectories", "n_tr", "n_tr_trajectories", file=dump)

def evaluate(result_sha,mail,list_sequences, eval_modified):
    """
    ...
    for c in ("car", "pedestrian"):
        e = trackingEvaluation(t_sha=result_sha, list_seq=list_sequences, mail=mail,cls=c)
        ...
        if e.compute3rdPartyMetrics(eval_modified):
            e.saveToStats(mail)
        else:
...
def run_mot_eval(result_path, list_sequences, eval_modified):  # eval modified indicates MOTA* usage
    # get unique sha key of submitted results
    result_sha = result_path
    mail = mailpy.Mail("")

    # evaluate results and send notification email to user
    success = evaluate(result_sha,mail,list_sequences, eval_modified)
